...
void compararProbabilidades(float* probabilidadesBayes, float* probabilidadesKNN){
    ...
    for (int i = 1; i < 40; i++){
        if (probabilidadesKNN[i] != 0){
            for (int j = i+1; j < 40; j++){
                if (probabilidadesKNN[j] != 0){
                    if (probabilidadesKNN[i] > probabilidadesKNN[j]){
                        porcentaje = probabilidadesKNN[i]/probabilidadesKNN[j];
                        resultados[i] = probabilidadesBayes[j] * porcentaje;
                    } else if (probabilidadesKNN[i] < probabilidadesKNN[j]) {
                        porcentaje = probabilidadesKNN[j]/probabilidadesKNN[i];
                        resultados[j] = probabilidadesBayes[i] * porcentaje;
                    }
                }
            }
        } else {
